...
import {palette, spacing, breaks} from '../themes/generalTheme';

function createData(day, hours, classType) {
  return { day, hours, classType };
}
...
const Schedule = ({schedule, className}) => {
  ...
  const rows = schedule?.map(ph => [createData(ph.day,Object.entries(ph.hours), Object.entries(ph.classType))]);

  return (

    <AllDaysWrapper className={`${query} ${className}`}>  
      {rows.map((row) => (
        <RowWrapper key={row.day}>
